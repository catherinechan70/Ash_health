...
setwd('D:/')
path2 = "Chan_Thesis_Missions/Ash_07262019/100131_ash_atkinson1_2019_07_27_13_58_10/raw_15366_rd_rf_or"
...
#new_prj<-"+proj=longlat +datum=WGS84 +no_defs"
new_prj<-crs(new_prj)
...
# test that the tile overlaps with the whole cube (that projection is same)
###plot(tst2_rgb$X692.025.nm) 
###plot(tst2_rgb_tiles[[2]], add=T) ### this overlaps
...
tst2_812_tiles<-splitRaster(tst2_812_proj, nx=5, ny=5)


#Define projection for each tile


prj_fnc<- function(x)
{
  prj_tile<-list()
  prj_tile[x] <-projectRaster(tst2_812_tiles[[x]], crs=new_prj2)
}

tiles_projected<-lapply(1:length(tst2_812_tiles), prj_fnc)

###tst2_812_tiles<-projectRaster(tst2_812_tiles[1], crs=new_prj)
...
plot(tst2_812_tiles[[1]])


tst2_tile2_seg_26983_winSize51_Dist500 <-itcIMG(tile2_raster, epsg = 26983, searchWinSize = 51, DIST = 500) #Throws projection error
###epsg = 3437
...
plot(tst2_rgb$X543.883.nm)
plot(tst2_tile2_seg_26983_winSize51_Dist500)
plot(tst2_rgb$X543.883.nm, add=T)


new_prj2 <- "+proj=longlat +datum=WGS84 +no_defs"
new_prj2<-crs(new_prj2)

seg_15366_t2_proj<-spTransform(tst2_tile2_seg_26983_winSize51_Dist500, CRS(new_prj2))
tst2_rgb_reprj <- spTransform(tst2_rgb, crs(new_prj))


plot(tst2_rgb$X543.883.nm)
plot(seg_15366_t2_proj)
plot(tst2_rgb$X543.883.nm, add=T)
...
### taken from below, (skipping function)
writeOGR(seg_15366_t2_proj, dsn = paste("Forests/RGB_images/seg_15366_t2_proj",".shp",sep=""), driver="ESRI Shapefile", layer="seg_15366_t2_proj")
writeOGR(tst2_tile1_seg_26983_winSize51_Dist500, dsn = paste("Forests/RGB_images/seg_15366_t1",".shp",sep=""), driver="ESRI Shapefile", layer="tst2_tile1_seg_26983_winSize51_Dist500")


#tst_seg_4326_winSize3<-spTransform(tst_seg_26983_winSize3_Dist50, CRS ="+proj=longlat +datum=WGS84")
#UNIT TEST
tile_seg_tst<-list()
tile_seg_tst[1] <-itcIMG(tst2_812_tiles[[1]], epsg = 26983, searchWinSize = 51, DIST = 500) #Throws projection error
